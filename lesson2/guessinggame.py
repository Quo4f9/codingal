...
def intro():
	...
	if(number%2==0): #% is the modulus operator, it checks with the remainder.here if the number devided by 2 gives a remainder 0 it means the number is an even number.
		x='even'
	else:
		x='odd'
		...
		try: 
			...
			if guess<=100 and guess>=1: #if they are in range
				...
				if guessesTaken<6:
					...
					if guess != number:
						time.sleep(.5) #wait for .5 seconds (500ms) 
						print("Try Again!")
